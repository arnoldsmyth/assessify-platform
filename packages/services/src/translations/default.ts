...
import { createTranslationService, type TranslationService } from './translation-service';
...
let instance: TranslationService | undefined;
...
export function getTranslationService(): TranslationService {
  if (!instance) {
    ...
    if (!connectionString) {
      throw new Error(
        'DATABASE_URL is not set — required for the default translation service wiring'
      );
    }
    const { db } = getDbHandle(connectionString);
    instance = createTranslationService({

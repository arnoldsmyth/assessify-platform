...
let instance: QuestionnaireVersionService | undefined;
 ...
 */
export function getQuestionnaireVersionService(): QuestionnaireVersionService {
  if (!instance) {
    const connectionString = process.env.DATABASE_URL;
    if (!connectionString) {
      throw new Error(
        'DATABASE_URL is not set — required for the default questionnaire version service wiring'
      );
    }
    const { db } = getDbHandle(connectionString);
    instance = createQuestionnaireVersionService({
